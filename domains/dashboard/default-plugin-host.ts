...
async function importLegacyProjectKnowledgeState(
  projectRoot: string,
  ...
  try {
    if (canonicalStore.hasCompletedMigration(LEGACY_DASHBOARD_CACHE_MIGRATION_KEY)) return;

    let snapshot;
    try {
      snapshot = await readProjectKnowledgeStoreSnapshot(legacyLocation.databasePath);
    } catch {
      return;
    }
...
export function createDefaultDashboardPluginHostFactory(
  ...
  return async (projectId, projectPath) => {
    if (stateRoot === undefined && knowledgeCacheRoot === undefined) {
      await importLegacyProjectKnowledgeState(
        projectPath,
        homeDirectory,
